  ...
  implements ViewPager.Decor
{
  ...
  void updateTextPositions(int paramInt, float paramFloat, boolean paramBoolean)
  {
    ...
    if (paramInt != mLastKnownCurrentPage)
    {
      ...
      j = mCurrText.getBaseline();
      i7 = mNextText.getBaseline();
      ...
      i8 = Math.max(Math.max(i6 + i9, j + i10), i7 + i8);
    ...
    for (;;)
    {
      mCurrText.layout(m, j, i5, mCurrText.getMeasuredHeight() + j);
      j = Math.min(i2, m - mScaledTextSpacing - i);
      mPrevText.layout(j, i4, j + i, mPrevText.getMeasuredHeight() + i4);
      j = Math.max(n - i3 - k, mScaledTextSpacing + i5);
      mNextText.layout(j, paramInt, j + k, mNextText.getMeasuredHeight() + paramInt);
