  ...
  implements ViewPager.Decor
{
  ...
  void updateText(int paramInt, PagerAdapter paramPagerAdapter)
  {
    ...
    for (Object localObject = paramPagerAdapter.getPageTitle(paramInt);; localObject = null)
    {
      ...
      j = View.MeasureSpec.makeMeasureSpec((int)((j - k - m) * 0.8F), Integer.MIN_VALUE);
      i = View.MeasureSpec.makeMeasureSpec(n - i - i1, Integer.MIN_VALUE);
      mPrevText.measure(j, i);
      mCurrText.measure(j, i);
      mNextText.measure(j, i);
  ...
  void updateTextPositions(int paramInt, float paramFloat, boolean paramBoolean)
  {
    ...
    for (;;)
    {
      mCurrText.layout(i5, i1, j, mCurrText.getMeasuredHeight() + i1);
      i1 = Math.min(i2, i5 - mScaledTextSpacing - i);
      mPrevText.layout(i1, i4, i1 + i, mPrevText.getMeasuredHeight() + i4);
      i1 = Math.max(n - i3 - k, mScaledTextSpacing + j);
      mNextText.layout(i1, paramInt, i1 + k, mNextText.getMeasuredHeight() + paramInt);
