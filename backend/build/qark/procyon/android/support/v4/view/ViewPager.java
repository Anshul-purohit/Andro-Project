...
public class ViewPager extends ViewGroup
{
    ...
    private void calculatePageOffsets(ItemInfo itemInfo, int i, ItemInfo itemInfo2) {
        ...
        while (index >= 0) {
            for (itemInfo2 = this.mItems.get(index); l > itemInfo2.position; --l) {
                n8 -= this.mAdapter.getPageWidth(l) + n;
            }
            n8 -= itemInfo2.widthFactor + n;
            itemInfo2.offset = n8;
            if (itemInfo2.position == 0) {
                this.mFirstOffset = n8;
            }
            --index;
            --l;
        }
        ...
        final int n10 = i + 1;
        i = n9;
        for (int index2 = n10; index2 < size; ++index2, ++i) {
    ...
    private void recomputeScrollPosition(int n, int n2, int duration, int timePassed) {
        if (n2 > 0 && !this.mItems.isEmpty()) {
            n2 = (int)((n - this.getPaddingLeft() - this.getPaddingRight() + duration) * (this.getScrollX() / (float)(n2 - this.getPaddingLeft() - this.getPaddingRight() + timePassed)));
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        if (this.mPageMargin > 0 && this.mMarginDrawable != null && this.mItems.size() > 0 && this.mAdapter != null) {
            ...
            for (int size = this.mItems.size(), i = itemInfo.position; i < this.mItems.get(size - 1).position; ++i) {
                ...
                if (this.mPageMargin + n2 > scrollX) {
                    this.mMarginDrawable.setBounds((int)n2, this.mTopPageBounds, (int)(this.mPageMargin + n2 + 0.5f), this.mBottomPageBounds);
            ...
            if (this.getWindowToken() != null) {
                ...
                if (addNewItem != null) {
                    ...
                    if (clientWidth <= 0) {
                        ...
                        n4 = 2.0f - addNewItem.widthFactor + this.getPaddingLeft() / (float)clientWidth;
                    }
                    ...
                    while (j >= 0) {
                        ...
                        if (n3 >= n4 && j < max) {
                            ...
                            n6 = n3 + this.addNewItem(j, index + 1).widthFactor;
                            i = n5 + 1;
                            if (index >= 0) {
                    ...
                    this.calculatePageOffsets(addNewItem, n5, infoForPosition);
                }
    ...
    void smoothScrollTo(int a, int n, int abs) {
        ...
        final int n3 = a / 2;
        final float min = Math.min(1.0f, 1.0f * Math.abs(n2) / a);
        ...
        abs = Math.abs(abs);
        if (abs > 0) {
            a = Math.round(1000.0f * Math.abs((n4 + n5 * distanceInfluenceForSnapDuration) / abs)) * 4;
        }
        else {
            a = (int)((1.0f + Math.abs(n2) / (this.mPageMargin + a * this.mAdapter.getPageWidth(this.mCurItem))) * 100.0f);
        }
        a = Math.min(a, 600);
        this.mScroller.startScroll(scrollX, scrollY, n2, n, a);
        ViewCompat.postInvalidateOnAnimation((View)this);
    }
