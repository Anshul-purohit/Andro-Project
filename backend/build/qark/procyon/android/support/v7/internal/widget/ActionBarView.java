...
public class ActionBarView extends AbsActionBarView
{
    ...
    protected void onMeasure(int i, int a) {
        ...
        Label_0102: {
            ...
            int n = 0;
            int n2;
            for (int j = 0; j < childCount; ++j, n = n2) {
        ...
        if (View$MeasureSpec.getMode(a) != Integer.MIN_VALUE) {
        ...
        if (homeView.getVisibility() != 8) {
            ...
            if (layoutParams.width < 0) {
                i = View$MeasureSpec.makeMeasureSpec(n5, Integer.MIN_VALUE);
            }
            else {
                i = View$MeasureSpec.makeMeasureSpec(layoutParams.width, 1073741824);
            }
            homeView.measure(i, View$MeasureSpec.makeMeasureSpec(b, 1073741824));
            i = homeView.getMeasuredWidth() + homeView.getLeftOffset();
            a = Math.max(0, n5 - i);
            max = Math.max(0, a - i);
        }
        int measureChildView = a;
        i = n6;
        if (this.mMenuView != null) {
            measureChildView = a;
            i = n6;
            if (this.mMenuView.getParent() == this) {
                measureChildView = this.measureChildView((View)this.mMenuView, a, measureSpec, 0);
                i = Math.max(0, n6 - this.mMenuView.getMeasuredWidth());
            }
        }
            ...
            switch (this.mNavigationMode) {
                ...
                case 1: {
                    ...
                    if (this.mListNavLayout != null) {
                        ...
                        a = Math.max(0, measureChildView2 - i);
                        final int max3 = Math.max(0, max - i);
                        this.mListNavLayout.measure(View$MeasureSpec.makeMeasureSpec(a, Integer.MIN_VALUE), View$MeasureSpec.makeMeasureSpec(b, 1073741824));
                        final int measuredWidth = this.mListNavLayout.getMeasuredWidth();
                        i = Math.max(0, a - measuredWidth);
                ...
                case 2: {
                    ...
                    if (this.mTabScrollView != null) {
                        ...
                        this.mTabScrollView.measure(View$MeasureSpec.makeMeasureSpec(a, Integer.MIN_VALUE), View$MeasureSpec.makeMeasureSpec(b, 1073741824));
                        final int measuredWidth2 = this.mTabScrollView.getMeasuredWidth();
                        i = Math.max(0, a - measuredWidth2);
                        a = Math.max(0, max4 - measuredWidth2);
                        break;
                    }
                    break;
                }
        ...
        if (view2 != null) {
            ...
            int n9 = 0;
            if (layoutParams2 != null) {
                n8 = layoutParams2.leftMargin + layoutParams2.rightMargin;
                n9 = layoutParams2.topMargin + layoutParams2.bottomMargin;
            }
            int n10;
            if (this.mContentHeight <= 0) {
            ...
            if (generateLayoutParams.height >= 0) {
                min = Math.min(generateLayoutParams.height, b);
            }
            final int max5 = Math.max(0, min - n9);
            int n11;
            ...
            final int max6 = Math.max(0, min2 - n8);
            int gravity;
            if (layoutParams2 != null) {
                gravity = layoutParams2.gravity;
            }
            ...
            if ((gravity & 0x7) == 0x1) {
                ...
                if (generateLayoutParams.width == -1) {
                    n12 = Math.min(a, max2) * 2;
                }
            }
            view2.measure(View$MeasureSpec.makeMeasureSpec(n12, n11), View$MeasureSpec.makeMeasureSpec(max5, n10));
            n7 = i - (view2.getMeasuredWidth() + n8);
        }
        if (this.mExpandedActionView == null && b2) {
            this.measureChildView((View)this.mTitleLayout, n7, View$MeasureSpec.makeMeasureSpec(this.mContentHeight, 1073741824), 0);
        ...
        if (this.mContentHeight <= 0) {
            ...
            int n13;
            int n14;
            for (i = 0; i < childCount; ++i, a = n14) {
                n13 = this.getChildAt(i).getMeasuredHeight() + n4;
                if (n13 > (n14 = a)) {
                    n14 = n13;
                }
            }
            this.setMeasuredDimension(size, a);
        }
        else {
    ...
    private static class HomeView extends FrameLayout
    {
        ...
        protected void onMeasure(int b, int b2) {
            this.measureChildWithMargins((View)this.mUpView, b, 0, b2, 0);
            final FrameLayout$LayoutParams frameLayout$LayoutParams = (FrameLayout$LayoutParams)this.mUpView.getLayoutParams();
            ...
            this.measureChildWithMargins((View)this.mIconView, b, mUpWidth, b2, 0);
            final FrameLayout$LayoutParams frameLayout$LayoutParams2 = (FrameLayout$LayoutParams)this.mIconView.getLayoutParams();
            final int a = mUpWidth + (frameLayout$LayoutParams2.leftMargin + this.mIconView.getMeasuredWidth() + frameLayout$LayoutParams2.rightMargin);
            ...
            final int mode2 = View$MeasureSpec.getMode(b2);
        ...
        public void setUpIndicator(final int mUpIndicatorRes) {
            ...
            if (mUpIndicatorRes != 0) {
                imageDrawable = this.getResources().getDrawable(mUpIndicatorRes);
            }
            else {
                imageDrawable = this.mDefaultUpIndicator;
            }
            mUpView.setImageDrawable(imageDrawable);
        }
