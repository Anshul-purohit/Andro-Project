...
class ActionBarImplICS extends ActionBar
{
    ...
    public void addOnMenuVisibilityListener(final OnMenuVisibilityListener onMenuVisibilityListener) {
        if (onMenuVisibilityListener != null) {
            final OnMenuVisibilityListenerWrapper referent = new OnMenuVisibilityListenerWrapper(onMenuVisibilityListener);
            this.mAddedMenuVisWrappers.add(new WeakReference<OnMenuVisibilityListenerWrapper>(referent));
            this.mActionBar.addOnMenuVisibilityListener((ActionBar$OnMenuVisibilityListener)referent);
        }
    }
    ...
    ImageView getHomeActionView() {
        if (this.mHomeActionView == null) {
            final View viewById = this.mActivity.findViewById(16908332);
            if (viewById != null) {
                ...
                if (viewGroup.getChildCount() == 2) {
                    ...
                    if (child instanceof ImageView) {
                        this.mHomeActionView = (ImageView)child;
    ...
    Drawable getThemeDefaultUpIndicator() {
        final TypedArray obtainStyledAttributes = this.mActivity.obtainStyledAttributes(new int[] { 16843531 });
    ...
    @Override
    public void setHomeAsUpIndicator(final int imageResource) {
        final ImageView homeActionView = this.getHomeActionView();
        if (homeActionView != null) {
            if (imageResource == 0) {
                homeActionView.setImageDrawable(this.getThemeDefaultUpIndicator());
                return;
            }
            homeActionView.setImageResource(imageResource);
        }
    }
    ...
    public void setHomeAsUpIndicator(final Drawable drawable) {
        final ImageView homeActionView = this.getHomeActionView();
        if (homeActionView != null) {
            Drawable themeDefaultUpIndicator;
            if ((themeDefaultUpIndicator = drawable) == null) {
                themeDefaultUpIndicator = this.getThemeDefaultUpIndicator();
            }
            homeActionView.setImageDrawable(themeDefaultUpIndicator);
        }
    }
