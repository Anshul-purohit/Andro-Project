...
abstract class ActionBarActivityDelegate {
   ...
   static ActionBarActivityDelegate createDelegate(ActionBarActivity var0) {
      if (VERSION.SDK_INT >= 18) {
         return new ActionBarActivityDelegateJBMR2(var0);
      } else if (VERSION.SDK_INT >= 16) {
         return new ActionBarActivityDelegateJB(var0);
      } else if (VERSION.SDK_INT >= 14) {
         return new ActionBarActivityDelegateICS(var0);
      } else {
         return (ActionBarActivityDelegate)(VERSION.SDK_INT >= 11 ? new ActionBarActivityDelegateHC(var0) : new ActionBarActivityDelegateBase(var0));
      }
   }
   ...
   void onCreate(Bundle var1) {
      TypedArray var2 = this.mActivity.obtainStyledAttributes(R$styleable.ActionBarWindow);
      if (!var2.hasValue(0)) {
         var2.recycle();
   ...
   private class ActionBarDrawableToggleImpl implements ActionBarDrawerToggle.Delegate {
      ...
      public Drawable getThemeUpIndicator() {
         TypedArray var1 = ActionBarActivityDelegate.this.mActivity.obtainStyledAttributes(new int[]{ActionBarActivityDelegate.this.getHomeAsUpIndicatorAttrId()});
