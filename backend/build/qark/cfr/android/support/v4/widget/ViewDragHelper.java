...
public class ViewDragHelper {
    ...
    private boolean checkTouchSlop(View view, float f, float f2) {
        ...
        if (bl2) {
            if (Math.abs((float)f) > (float)this.mTouchSlop) return bl;
            return false;
        }
        if (!bl3) {
            return false;
        }
        if (Math.abs((float)f2) > (float)this.mTouchSlop) return bl;
        return false;
    }
    ...
    private int computeAxisDuration(int n, int n2, int n3) {
        ...
        if ((n2 = Math.abs((int)n2)) > 0) {
            n = Math.round((float)(1000.0f * Math.abs((float)((f2 + f3 * f) / (float)n2)))) * 4;
            do {
                return Math.min((int)n, (int)600);
                break;
            } while (true);
        }
        n = (int)(((float)Math.abs((int)n) / (float)n3 + 1.0f) * 256.0f);
        return Math.min((int)n, (int)600);
    }
        ...
        switch (var7_2) {
            ...
            case 2: {
                ...
                do {
                    ...
                    if (this.mDragState != 1 && (!this.checkTouchSlop(var11_18 = this.findTopChildUnder((int)var2_7, (int)var3_12), var4_14, var5_15) || !this.tryCaptureViewForDrag(var11_18, var8_16))) ** GOTO lbl59
lbl57: // 2 sources:
                    this.saveLastMotion(var1_1);
                    return;
lbl59: // 1 sources:
    ...
    public boolean shouldInterceptTouchEvent(MotionEvent motionEvent) {
        ...
        switch (n) {
            ...
            case 2: {
                n = MotionEventCompat.getPointerCount(motionEvent);
                for (n2 = 0; n2 < n; ++n2) {
                    View view;
                    int n3 = MotionEventCompat.getPointerId(motionEvent, n2);
                    float f = MotionEventCompat.getX(motionEvent, n2);
